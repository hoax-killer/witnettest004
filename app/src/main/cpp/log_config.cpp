    ...
    case SET_MASK:
        {
            ...
            if (equip_id == -1) {
                ...
                int mask_len = (highest / 8) + 1; 								// сколько байт нам нужно чтобы обозначить целую часть по модулю 8 самого большого item_id
                ...
                *((int *)(buf.first + 12)) = highest + 1;
                ...
                for (int i = 0; i < mask_len; i++)
                    mask[i] = 0;
/*
я так понимаю здесь происходит упаковка числовых кодов в маску следующим образом:
маска - это массив байтов.
индекс ячейки в этом массиве определяет на сколько раз надо умножить число 8 чтобы получить числовой код команды о модулю 8
каждый бит внутри данной ячейки о пределяет число, которое надо добавить к вышеописанному умножению для получения остатка.
Таким образом код команды разбит на целочисленное деление и остаток от этого деления. Целочисленное деление кодируется индексом массива
а остаток от деления - битом внутри ячейки массива.
*/
                for (size_t i = 0; i < type_ids.size(); i++) {
                    int id = type_ids[i];
